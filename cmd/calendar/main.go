...
import (
	...
	"github.com/thalestmm/ots/api/calendarserver"
...
func main() {
	...
	h := calendarserver.NewHandler(agg, cal, version)
	...
	srv := &http.Server{
		Addr:              *addr,
		Handler:           calendarserver.RequestLogging(log, mux),
		ReadHeaderTimeout: 10 * time.Second,
	}

	go func() {
		log.Info("ots calendar server listening", "addr", *addr)
